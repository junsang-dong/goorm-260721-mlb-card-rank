...
## 이번 작업 요약 (Apify eBay Scraper 연동)

[Apify Console](https://console.apify.com/)의 [eBay Scraper Actor](https://console.apify.com/actors/Y7h6Aodb7ZXkv6Ieb/input)(`Y7h6Aodb7ZXkv6Ieb` / `automation-lab/ebay-scraper`)로 Best Match 인기 카드 20개를 수집하고, 기존 Playwright 목록 **위에** Apify 섹션으로 표시합니다. 가이드: [doc/GUIDE Apify eBay Scraper.md](doc/GUIDE%20Apify%20eBay%20Scraper.md)

1. `apify-client`로 Actor 실행 → JSON 정규화 → Turso 저장 (`source=apify`, id=`apify-{itemId}`)
2. 이어서 Playwright 스크래핑 유지 (`source=playwright`, id=`ebay-{itemId}`)
3. 미분석 카드 GPT 분석 + `rankingScore` 계산
4. UI: **Apify eBay Scraper** 섹션 → **Playwright** 섹션 순서로 4열 카드 그리드 노출
5. 로컬 전용: `npm run scrape:apify` / `npm run scrape:full`
6. `/api/cards`가 `apifyCards` / `playwrightCards`를 분리 반환하고, 홈에서 각각 렌더

검색어 기본값: `MLB sports card` (`APIFY_SEARCH_QUERY`로 변경 가능)

### Apify 연동에서 정리·수정한 사항

- **소스 구분 컬럼**: `cards.source` (`apify` | `playwright`) 추가. 기존 Turso DB는 `migrate.ts`의 `ALTER TABLE`로 안전하게 보강 (이미 있으면 duplicate column 무시).
- **ID 충돌 방지**: 동일 eBay item이어도 `apify-{id}` / `ebay-{id}`로 분리 저장해 두 파이프라인이 서로를 덮어쓰지 않음.
- **부분 실패 허용**: Apify 토큰 누락·Actor 실패 시에도 Playwright 수집은 계속 진행. 양쪽 모두 실패할 때만 스크래프를 치명적 오류로 처리.
- **Apify 필드 파싱**: `sellerFeedbackPercent`(`99.3%`), `sellerFeedbackCount`(`153.4K`), `soldCount`(`93+`), shipping(`Free delivery` / `+$14.99`)을 숫자로 정규화.
- **타입 연산자 혼용 오류**: `??`와 `||` 혼용 TS5076을 괄호로 분리해 서버 타입체크 통과.
- **스크래프 타임아웃**: Apify 대기 + Playwright + GPT를 고려해 `api/scrape` `maxDuration`을 60 → 300초로 상향.
- **환경 변수**: `.env.example`에 `APIFY_TOKEN` / `APIFY_ACTOR_ID` / `APIFY_SEARCH_QUERY` 문서화 (실제 토큰은 `.env`에만 두고 커밋하지 않음).

## 이번 작업 요약 (UI 개선)

테이블 기반 랭킹 화면을 **카드 그리드**로 바꾸고, Card / Team / Brand / Year / Price 기준 **정렬**을 추가했습니다.

1. `RankingTable` / `CardRow` 제거 → `RankingGrid` / `CardItem`으로 교체
2. 반응형 그리드: 모바일 1열 · 태블릿 2열 · 대형 3열 · 와이드 4열
...
5. 레이아웃 최대 너비를 `max-w-7xl`로 확대해 4열 카드가 여유 있게 보이도록 조정
...
scripts/
  ...
  dev-apify-scrape.ts       Apify Actor만 실행 → DB → GPT 분석
  dev-full-scrape.ts        Apify + Playwright → DB → GPT 분석
```
...
```bash
npm install
cp .env.example .env   # OPENAI / TURSO / APIFY_TOKEN 채우기
npm run db:migrate     # Turso에 스키마 생성
npm run scrape:apify   # Apify로 20개 수집 + 분석 (토큰 필요)
...
SCRAPE_LIMIT=20
APIFY_TOKEN=
APIFY_ACTOR_ID=Y7h6Aodb7ZXkv6Ieb
APIFY_SEARCH_QUERY=MLB sports card
```

Apify 토큰은 [Apify Console](https://console.apify.com/) → Settings → Integrations에서 발급합니다. Actor 입력 UI: [eBay Scraper input](https://console.apify.com/actors/Y7h6Aodb7ZXkv6Ieb/input).

## 알려진 제약 사항 (다음 단계)

- `@sparticuz/chromium`은 Linux 전용이라 `/api/scrape`의 Playwright 경로는 Vercel 배포 환경에서 검증하는 것이 안전합니다 (로컬은 `scripts/dev-full-scrape.ts` / `scrape:apify`로 대체 검증).
- Vercel Cron(`vercel.json`)도 배포 후 대시보드에서 실제 실행 여부를 확인해야 합니다. Cron에 Apify를 쓰려면 Vercel 환경 변수에 `APIFY_TOKEN`이 필요합니다.
...
- 수집 키워드가 `MLB sports card` 기본값이라 선수별(예: Ohtani) 인기 카드가 Best Match TOP20에 없으면 랭킹에 나타나지 않을 수 있습니다. `APIFY_SEARCH_QUERY`로 선수별 검색은 가능합니다.
