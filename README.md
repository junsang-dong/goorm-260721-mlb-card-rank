...
이 README는 스펙 문서를 기반으로 MVP를 처음 구현한 세션의 결과물을 정리합니다.
...
4. 랭킹 스코어 계산 후 React + Vite UI(TOP20 테이블)로 노출
...
src/
  pages/HomePage.tsx      Hero + TOP20 테이블
  components/             RankingTable, CardRow, LoadingState, ErrorState
...
- 관리자 페이지, CSV 다운로드, 카드 상세 페이지 UI, 필터(Player/Team/Brand/Year/Price/Rookie/PSA)는 스펙에 정의되어 있으나 이번 MVP 범위에서 제외했습니다.
